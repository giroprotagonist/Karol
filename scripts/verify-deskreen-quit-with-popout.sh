#!/usr/bin/env bash
# Smoke test: pop-out queue window closes cleanly on quit (no zombie window, port released).
set -euo pipefail
...
echo "=== Deskreen Quit + Pop-out Smoke Test ==="
...
echo ""
echo "--- Open pop-out queue window via API ---"
OPEN_JSON="$(curl -sf -X POST "$BASE/api/youtube-dj/queue-window/open" \
	-H 'Content-Type: application/json' -d '{}')"
OPEN="$(echo "$OPEN_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('open',False))" 2>/dev/null || echo False)"
if [ "$OPEN" != "True" ] && [ "$OPEN" != "true" ]; then
	echo "FAIL: queue-window did not open (response: $OPEN_JSON)"
	exit 1
fi
echo "PASS: pop-out queue window open"

sleep 1
STATUS_JSON="$(curl -sf "$BASE/api/youtube-dj/queue-window")"
STILL_OPEN="$(echo "$STATUS_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('open',False))" 2>/dev/null || echo False)"
if [ "$STILL_OPEN" != "True" ] && [ "$STILL_OPEN" != "true" ]; then
	echo "FAIL: queue-window status reports closed immediately"
	exit 1
fi
...
# Reuse base quit script checks if available
if [ -x "$SCRIPT_DIR/verify-deskreen-quit.sh" ]; then
	echo ""
	echo "(Base quit script also available at scripts/verify-deskreen-quit.sh)"
fi

echo "=== Quit + pop-out smoke test complete ==="
