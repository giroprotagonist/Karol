...
class KarolUrlTest {
	...
	fun normalize_addsSchemeAndDjControllerPath() {
		assertEquals(
			...
			KarolUrl.normalize("192.168.1.42:3131"),
		)
	}
	...
	fun normalize_preservesExistingDjControllerUrl() {
		assertEquals(
			"http://192.168.1.42:3131/dj-controller/",
			KarolUrl.normalize("http://192.168.1.42:3131/dj-controller/"),
		)
	}
	...
	fun normalize_defaultsPortWhenMissing() {
		assertEquals(
			...
			KarolUrl.normalize("http://tablet.local"),
		)
	}

	@Test
	fun normalize_rejectsBlankInput() {
		assertNull(KarolUrl.normalize("   "))
	}
}
